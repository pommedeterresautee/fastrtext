...
test_that("Unsupervised training", {
  ...
  try(execute(commands = c("skipgram", "-input", tmp_file_txt, "-output", tmp_file_model, "-verbose", 0)), silent = TRUE)
...
test_that("Supervised training", {
 ...
 tmp_file_model <- tempfile()
  ...
  # learn model
  try(execute(commands = c("supervised", "-input", train_tmp_file_txt, "-output", tmp_file_model, "-dim", 20, "-lr", 1, "-epoch", 20, "-wordNgrams", 2, "-verbose", 0)), silent = TRUE)
